...
FORM GET_DATA_/SEW/INT_FC_V.
  PERFORM VIM_FILL_WHERETAB.
*.read data from database.............................................*
  REFRESH TOTAL.
  CLEAR   TOTAL.
  SELECT * FROM /SEW/INT_FIE_GEN WHERE
(VIM_WHERETAB) .
    CLEAR /SEW/INT_FC_V .
/SEW/INT_FC_V-MANDT =
/SEW/INT_FIE_GEN-MANDT .
/SEW/INT_FC_V-MOLGA =
/SEW/INT_FIE_GEN-MOLGA .
/SEW/INT_FC_V-INFTY =
/SEW/INT_FIE_GEN-INFTY .
/SEW/INT_FC_V-FIELD =
/SEW/INT_FIE_GEN-FIELD .
/SEW/INT_FC_V-CHANGEABLE_TO =
/SEW/INT_FIE_GEN-CHANGEABLE_TO .
/SEW/INT_FC_V-LAST_CHANGED_AT =
/SEW/INT_FIE_GEN-LAST_CHANGED_AT .
/SEW/INT_FC_V-LAST_CHANGED_BY =
/SEW/INT_FIE_GEN-LAST_CHANGED_BY .
<VIM_TOTAL_STRUC> = /SEW/INT_FC_V.
    APPEND TOTAL.
  ENDSELECT.
  SORT TOTAL BY <VIM_XTOTAL_KEY>.
  <STATUS>-ALR_SORTED = 'R'.
...
ENDFORM.
*---------------------------------------------------------------------*
FORM DB_UPD_/SEW/INT_FC_V .
*.process data base updates/inserts/deletes.........................*
LOOP AT TOTAL.
  CHECK <ACTION> NE ORIGINAL.
MOVE <VIM_TOTAL_STRUC> TO /SEW/INT_FC_V.
  IF <ACTION> = UPDATE_GELOESCHT.
    <ACTION> = GELOESCHT.
  ENDIF.
  CASE <ACTION>.
   WHEN NEUER_GELOESCHT.
IF STATUS_/SEW/INT_FC_V-ST_DELETE EQ GELOESCHT.
     READ TABLE EXTRACT WITH KEY <VIM_XTOTAL_KEY>.
     IF SY-SUBRC EQ 0.
       DELETE EXTRACT INDEX SY-TABIX.
     ENDIF.
    ...
    ENDIF.
   WHEN GELOESCHT.
  SELECT SINGLE FOR UPDATE * FROM /SEW/INT_FIE_GEN WHERE
  MOLGA = /SEW/INT_FC_V-MOLGA AND
  INFTY = /SEW/INT_FC_V-INFTY AND
  FIELD = /SEW/INT_FC_V-FIELD .
    IF SY-SUBRC = 0.
    DELETE /SEW/INT_FIE_GEN .
    ENDIF.
    IF STATUS-DELETE EQ GELOESCHT.
      READ TABLE EXTRACT WITH KEY <VIM_XTOTAL_KEY> BINARY SEARCH.
      DELETE EXTRACT INDEX SY-TABIX.
...
/SEW/INT_FC_V-LAST_CHANGED_BY .
    IF SY-SUBRC = 0.
    UPDATE /SEW/INT_FIE_GEN ##WARN_OK.
    ELSE.
    INSERT /SEW/INT_FIE_GEN .
    ENDIF.
    READ TABLE EXTRACT WITH KEY <VIM_XTOTAL_KEY>.
    IF SY-SUBRC EQ 0.
      <XACT> = ORIGINAL.
  ...
  ENDCASE.
ENDLOOP.
CLEAR: STATUS_/SEW/INT_FC_V-UPD_FLAG,
STATUS_/SEW/INT_FC_V-UPD_CHECKD.
MESSAGE S018(SV).
ENDFORM.
*---------------------------------------------------------------------*
FORM READ_SINGLE_/SEW/INT_FC_V.
  SELECT SINGLE * FROM /SEW/INT_FIE_GEN WHERE
MOLGA = /SEW/INT_FC_V-MOLGA AND
INFTY = /SEW/INT_FC_V-INFTY AND
FIELD = /SEW/INT_FC_V-FIELD .
/SEW/INT_FC_V-MANDT =
...
ENDFORM.
*---------------------------------------------------------------------*
FORM CORR_MAINT_/SEW/INT_FC_V USING VALUE(CM_ACTION) RC.
  DATA: RETCODE LIKE SY-SUBRC, COUNT TYPE I, TRSP_KEYLEN TYPE SYFLENG.
  FIELD-SYMBOLS: <TAB_KEY_X> TYPE X.
  CLEAR RC.
MOVE /SEW/INT_FC_V-MOLGA TO
/SEW/INT_FIE_GEN-MOLGA .
MOVE /SEW/INT_FC_V-INFTY TO
/SEW/INT_FIE_GEN-INFTY .
MOVE /SEW/INT_FC_V-FIELD TO
/SEW/INT_FIE_GEN-FIELD .
MOVE /SEW/INT_FC_V-MANDT TO
/SEW/INT_FIE_GEN-MANDT .
  CORR_KEYTAB             =  E071K.
  CORR_KEYTAB-OBJNAME     = '/SEW/INT_FIE_GEN'.
  IF NOT <vim_corr_keyx> IS ASSIGNED.
    ASSIGN CORR_KEYTAB-TABKEY TO <vim_corr_keyx> CASTING.
  ENDIF.
  ASSIGN /SEW/INT_FIE_GEN TO <TAB_KEY_X> CASTING.
  PERFORM VIM_GET_TRSPKEYLEN
    USING '/SEW/INT_FIE_GEN'
    CHANGING TRSP_KEYLEN.
  <VIM_CORR_KEYX>(TRSP_KEYLEN) = <TAB_KEY_X>(TRSP_KEYLEN).
  PERFORM UPDATE_CORR_KEYTAB USING CM_ACTION RETCODE.
  ADD: RETCODE TO RC, 1 TO COUNT.
